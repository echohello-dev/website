...
The site automatically generates an `llms.txt` file following the [llms.txt specification](https://llmstxt.org):

- **Script**: `scripts/generate-llms-txt.js`
- **Generated files**: 
  - `public/llms.txt` (accessible at `/llms.txt`)
  - `public/README.md` and `public/AGENTS.md` (for direct markdown access)
- **Auto-runs**: Before every dev/build via npm scripts
- **Dynamic content**: Reads from:
  - MDX files in `content/pages/` and `content/projects/`
  - Featured projects from `lib/data.ts`
  - Actual page descriptions and metadata
- **Format**: Markdown with required H1, blockquote summary, and H2 sections with link lists

The llms.txt provides LLMs with a curated, always-up-to-date overview including:
- All site pages with descriptions
